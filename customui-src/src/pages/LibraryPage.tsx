    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          Official <code>@seamos</code> libraries available for SeamOS Custom UI
          app development, and the criteria for choosing them.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-blue-200 bg-blue-50 p-6 text-sm text-blue-900">
        ...
        <ul className="mt-3 list-inside list-disc space-y-1">
          <li>
            For REST/WebSocket communication, use the built-in{' '}
            <code>@seamos/connect</code>.
          </li>
          <li>
            Apps that need a map UI use <code>@seamos/map-preset</code>,{' '}
            <code>maplibre-gl</code>, and <code>pmtiles</code>. They are included
            in this template for the runnable sample; if you do not need map
            features, you may remove the <code>/map</code> route and its related
            dependencies.
          </li>
          <li>
            For Cockpit WebView native integration, use{' '}
            <code>@seamos/bridge</code>. It is included in this template for the
            WebView sample.
          </li>
          <li>
            <code>@seamos/websocket</code> is deprecated. Do not add it to new
            apps — migrate to <code>@seamos/connect</code> instead.
          </li>
        </ul>
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          The default helper library that automatically resolves the port
          assigned by the SeamOS runtime and connects REST and WebSocket
          communication.
        </p>
...
client.close(1000, 'done')`}
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          A map preset that uses MapLibre GL and PMTiles to switch between dev
          (S3) and prod (device-local <code>/maps</code>) environments. See the
          runnable example at the <code>/map</code> route.
        </p>
        <div className="mt-4">
  ...
  return <div ref={ref} className="h-[420px]" />
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          ...
          WebView and the Custom UI. See the runnable example at the{' '}
...
unsubscribe()`}
        ...
        <div className="mt-4 rounded-md bg-yellow-50 p-4 text-sm text-yellow-800">
          In a regular browser there is no ReactNativeWebView, so native
          requests are not actually processed. Verify in the SeamOS Cockpit
          WebView or a test harness.
        </div>
      </div>
    </div>
  )
