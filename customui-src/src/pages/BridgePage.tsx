    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          An example that exchanges settings, location, haptic, vibration, file
          download, and custom messages with the SeamOS Cockpit or React Native
          WebView using <code>@seamos/bridge/webview</code>.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        {status === 'available' ? (
          ...
          <div className="mt-2 rounded-md bg-yellow-50 p-4 text-sm text-yellow-800">
            In a regular browser / Vite dev server there is no
            ReactNativeWebView, so native actions are not performed. Verify in
            the real SeamOS Cockpit WebView or a test harness.
          </div>
        )}
      </div>
