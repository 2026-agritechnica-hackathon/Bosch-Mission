...
export function ExternalApiPage() {
  ...
  const [responseText, setResponseText] = useState('The response will appear here.')
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResponseText('The response will appear here.')
    } finally {
      setPending(false)
    }
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          ...
          backend. The backend calls the external API through the Cloud plugin
          and pushes the response back over WebSocket (asynchronous Pattern B).
        </p>
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        {connState === 'ready' && (
          <p className="text-sm text-green-700">
            Initialized — assigned port: {assignedPort} · WebSocket{' '}
        ...
        {connState === 'error' && (
          <div className="space-y-2 text-sm text-red-700">
            ...
            <p>
              ...
              expected. Test again on a real SeamOS runtime.
            </p>
          </div>
        )}
      </div>
