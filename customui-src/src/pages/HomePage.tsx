...
export function HomePage() {
  ...
  const [restResult, setRestResult] = useState<string>(
    'REST response will appear here.',
  )
  ...
  const fetchStatus = async () => {
    ...
    setRestResult('GET /api/example/status in progress...')
  ...
  const sendCommand = async () => {
    ...
    setRestResult('POST /api/example/commands in progress...')
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          SeamOS REST/WebSocket communication example page using @seamos/connect
        </p>
        <p className="mt-2 text-gray-600">
          The SeamOS runtime must provide the <code>get_assigned_ports</code>{' '}
          endpoint, and the template examples connect REST and WebSocket through
          the assigned port.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        {connectionState === 'initializing' && (
          <p className="text-sm text-gray-600">
            Initializing port information...
          </p>
        )}
        {connectionState === 'ready' && (
          <p className="text-sm text-green-700">
            Initialized — assigned port: {assignedPort}
          </p>
        )}
        {connectionState === 'error' && (
          <div className="space-y-2 text-sm text-red-700">
            ...
            <p>
              ...
              expected. Test again on a real SeamOS runtime or a dev proxy that
              provides <code>get_assigned_ports</code>.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mb-4 text-sm text-gray-600">
          <code>seamosFetch(path, init)</code> sends HTTP requests through the
          assigned port. The example endpoints are placeholders that the app
          developer should replace to match the real API.
        </p>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mb-4 text-sm text-gray-600">
          Pass only a path with a leading slash, like{' '}
        ...
        <ul className="mt-4 space-y-1 text-sm text-gray-600">
          {wsLog.length === 0 ? (
            <li>WebSocket logs will appear here.</li>
          ) : (
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div className="space-y-3 text-sm text-gray-600">
          ...
          <div className="flex items-start gap-3">
            ...
            <span>
              — Type-safe file-based routing. Supports static deployment via
              hash routing
            </span>
          </div>
          <div className="flex items-start gap-3">
          ...
          <div className="flex items-start gap-3">
            ...
            <span>— Build tool. Supports HMR, code splitting, relative-path builds</span>
          </div>
          <div className="flex items-start gap-3">
            <span className="font-mono font-semibold text-gray-900">
              @seamos/connect
